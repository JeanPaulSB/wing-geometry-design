...
class Wing:
    ...
    def add_surface(self, surface):
        ...
        plt.plot(
            (surface.A.x, surface.B.x), (surface.A.y, surface.B.y), linestyle="dashed"
        )
        plt.plot(
            (surface.C.x, surface.D.x), (surface.C.y, surface.D.y), linestyle="dashed"
        )
    ...
    def plot(self):
        ...
        if len(self.surfaces) >= 1:
            ...
            plt.plot(
                (first_surface.A.x, first_surface.C.x),
                (first_surface.A.y, first_surface.C.y),
            )
            plt.plot(
                (last_surface.B.x, last_surface.D.x),
                (last_surface.B.y, last_surface.D.y),
            )
        plt.show()
